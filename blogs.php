...
<div class="flex flex-col lg:flex-row gap-8">
            ...
            <div class="bg-white border rounded-xl overflow-hidden shadow hover:shadow-lg transition">
                ...
                <div class="p-5">
                    ...
                    <h3 class="text-lg font-bold mt-1 mb-2">
                        <a href="<?php echo getBlogPostUrl($post); ?>" class="text-gray-900 hover:text-blue-600"><?php echo htmlspecialchars($post['title']); ?></a>
                    </h3>
                    ...
                    <div class="flex items-center justify-between text-xs text-gray-400">
                        ...
                        <a href="<?php echo getBlogPostUrl($post); ?>" class="text-blue-600 hover:underline font-medium">Read More</a>
                    </div>
                </div>
            </div>
            <?php endwhile; ?>
    ...
    <div class="w-full lg:w-80 space-y-6">
        ...
        <div class="bg-white border rounded-xl p-6 shadow-sm">
            <h3 class="font-bold text-gray-900 mb-3 text-sm">Search</h3>
            <form method="GET" action="/blog">
                <?php if ($cat_slug): ?><input type="hidden" name="category" value="<?php echo htmlspecialchars($cat_slug); ?>"><?php endif; ?>
                <div class="flex gap-2">
                    <input type="text" name="search" value="<?php echo htmlspecialchars($search_q); ?>" placeholder="Search posts..." class="flex-1 border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500">
        ...
        <!-- Categories -->
        <div class="bg-white border rounded-xl p-6 shadow-sm">
            <h3 class="font-bold text-gray-900 mb-3 text-sm">Categories</h3>
            <div class="space-y-2">
                ...
                while ($cat = mysqli_fetch_assoc($categories)):
                    $count = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as c FROM blog_posts WHERE category_id = {$cat['id']} AND status = 1"))['c'] ?? 0;
                ?>
                <a href="/blog/category/<?php echo urlencode($cat['slug']); ?>" class="block text-sm <?php echo $cat_slug === $cat['slug'] ? 'text-blue-600 font-medium' : 'text-gray-600 hover:text-blue-600'; ?>">
                    <?php echo htmlspecialchars($cat['name']); ?> (<?php echo $count; ?>)
                </a>
                <?php endwhile; ?>
            </div>
        </div>
