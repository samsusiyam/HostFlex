...
<div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    <div>
        <h1 class="text-2xl font-bold text-gray-800">Login Logs</h1>
        <p class="text-gray-500">Track all login attempts to the admin panel</p>
    </div>
    <?php if (!$no_table): ?>
    <div class="flex gap-2">
        <a href="?clear=1" onclick="return confirm('Clear all login logs?')" class="bg-red-600 text-white px-4 py-2 rounded text-sm hover:bg-red-700"><i class="fa fa-trash mr-1"></i> Clear Logs</a>
    </div>
    <?php endif; ?>
</div>
...
<div class="bg-white rounded-lg shadow overflow-hidden">
    <div class="p-4 border-b flex items-center justify-between">
        <form method="GET" class="flex gap-2">
            <input type="text" name="search" value="<?php echo htmlspecialchars($search); ?>" placeholder="Search by username or IP..." class="border rounded px-3 py-2 text-sm">
            <button type="submit" class="bg-blue-600 text-white px-3 py-2 rounded text-sm"><i class="fa fa-search"></i></button>
            <?php if ($search): ?><a href="login-logs.php" class="bg-gray-300 text-gray-700 px-3 py-2 rounded text-sm">Clear</a><?php endif; ?>
        </form>
    ...
    <table class="w-full">
        <thead class="bg-gray-50 border-b">
            <tr>
                ...
                <th class="text-left px-4 py-3 text-sm font-semibold text-gray-600">IP Address</th>
                <th class="text-left px-4 py-3 text-sm font-semibold text-gray-600">User Agent</th>
        ...
        <tbody class="divide-y">
            ...
            <tr class="hover:bg-gray-50 text-sm">
                ...
                <td class="px-4 py-3">
                    <span class="text-xs px-2 py-1 rounded-full <?php echo $log['status'] == 'success' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'; ?>"><?php echo ucfirst($log['status']); ?></span>
                </td>
                <td class="px-4 py-3 text-gray-500 font-mono"><?php echo htmlspecialchars($log['ip_address']); ?></td>
                <td class="px-4 py-3 text-gray-500 max-w-xs truncate" title="<?php echo htmlspecialchars($log['user_agent'] ?? ''); ?>"><?php echo htmlspecialchars(substr($log['user_agent'] ?? '-', 0, 50)); ?>...</td>
                <td class="px-4 py-3 text-gray-500 whitespace-nowrap"><?php echo date('d M Y H:i', strtotime($log['created_at'])); ?></td>
            </tr>
            <?php endwhile; else: ?>
            <tr><td colspan="5" class="px-4 py-8 text-center text-gray-400">No login logs yet.</td></tr>
            <?php endif; ?>
        </tbody>
    </table>
</div>
