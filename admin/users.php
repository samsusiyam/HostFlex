...
<div class="mb-6 flex items-center justify-between">
    <div>
        ...
        <p class="text-gray-500">Manage admin panel users and roles</p>
    </div>
    <a href="?edit=0" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 text-sm <?php echo $edit_user ? 'hidden' : ''; ?>"><i class="fa fa-plus mr-1"></i> New User</a>
</div>
...
<div class="bg-white rounded-lg shadow p-6 mb-6">
    ...
    <form method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-2xl">
            ...
            <div><label class="block text-sm font-medium text-gray-700 mb-1">Email</label><input type="email" name="email" value="<?php echo htmlspecialchars($eu['email']); ?>" required class="w-full border rounded px-3 py-2"></div>
            <div><label class="block text-sm font-medium text-gray-700 mb-1">Password <?php echo $is_new ? '' : '(leave blank to keep)'; ?></label><input type="password" name="password" class="w-full border rounded px-3 py-2" <?php echo $is_new ? 'required' : ''; ?>></div>
            ...
            <div class="flex items-center gap-2">
                <input type="checkbox" name="status" value="1" id="userStatus" <?php echo $eu['status'] ? 'checked' : ''; ?> class="h-5 w-5 text-blue-600 border rounded">
                <label for="userStatus" class="text-sm font-medium text-gray-700">Active</label>
            </div>
        </div>
        <button type="submit" name="save_user" class="mt-4 bg-blue-600 text-white px-5 py-2 rounded hover:bg-blue-700"><i class="fa fa-save mr-1"></i> <?php echo $is_new ? 'Create User' : 'Update User'; ?></button>
    </form>
</div>
<?php endif; ?>
...
<div class="bg-white rounded-lg shadow overflow-hidden">
    <table class="w-full">
        <thead class="bg-gray-50 border-b">
            <tr>
                <th class="text-left px-4 py-3 text-sm font-semibold text-gray-600">Username</th>
        ...
        <tbody class="divide-y">
            ...
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-4 py-3 text-sm text-gray-500"><?php echo htmlspecialchars($u['email']); ?></td>
                <td class="px-4 py-3">
                    <span class="text-xs px-2 py-1 rounded-full <?php echo ($u['role']??'') == 'admin' ? 'bg-purple-100 text-purple-700' : (($u['role']??'') == 'manager' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-700'); ?>"><?php echo ucfirst($u['role']??'editor'); ?></span>
                </td>
                <td class="px-4 py-3">
                    <span class="text-xs px-2 py-1 rounded-full <?php echo ($u['status']??1) ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'; ?>"><?php echo ($u['status']??1) ? 'Active' : 'Inactive'; ?></span>
                </td>
                <td class="px-4 py-3 text-sm text-gray-500"><?php echo date('d M Y', strtotime($u['created_at'])); ?></td>
                <td class="px-4 py-3 text-right">
                    <a href="?edit=<?php echo $u['id']; ?>" class="text-blue-600 hover:text-blue-800 mr-2"><i class="fa fa-edit"></i></a>
                    <?php if ($u['id'] != $_SESSION['admin_id']): ?>
                    <a href="?delete=<?php echo $u['id']; ?>" onclick="return confirm('Delete this user?')" class="text-red-600 hover:text-red-800"><i class="fa fa-trash"></i></a>
                    <?php endif; ?>
                </td>
            </tr>
            <?php endwhile; ?>
