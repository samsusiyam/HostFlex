...
<div class="mb-6"><h1 class="text-2xl font-bold text-gray-800">FAQs</h1><p class="text-gray-500">Frequently asked questions</p></div>
    ...
    <div class="bg-white rounded-lg shadow p-6">
        ...
        <form method="POST" id="itemForm">
            ...
            <div class="space-y-3">
                ...
                <div><label class="block text-sm font-medium text-gray-700 mb-1">Answer</label><textarea name="answer" id="fAnswer" rows="5" required class="w-full border rounded px-3 py-2"></textarea></div>
    ...
    <div class="lg:col-span-2 bg-white rounded-lg shadow overflow-hidden">
        <table class="w-full">
            <thead class="bg-gray-50 border-b">
                <tr><th class="text-left px-4 py-3 text-sm font-semibold text-gray-600">Question</th><th class="text-left px-4 py-3 text-sm font-semibold text-gray-600">Answer</th><th class="text-right px-4 py-3 text-sm font-semibold text-gray-600">Actions</th></tr>
            </thead>
            <tbody class="divide-y">
                <?php while ($row = mysqli_fetch_assoc($items)): ?>
                <tr class="hover:bg-gray-50">
                    <td class="px-4 py-3 text-sm font-medium max-w-xs truncate"><?php echo htmlspecialchars($row['question']); ?></td>
                    <td class="px-4 py-3 text-sm text-gray-500 max-w-md truncate"><?php echo strip_tags(substr($row['answer'], 0, 100)); ?>...</td>
